...
void game_target_render(game_target_t* target)
{
    ...
    targetRenderable->order = target->object->id + target->object->renderProperties.order;
    ...
    targetNeedleRenderable->order = target->needle->object->id + target->needle->object->renderProperties.order;
